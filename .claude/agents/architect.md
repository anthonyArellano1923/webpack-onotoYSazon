...
- **Arquitectura**: SPA React + Webpack con backend Express/Node.js desacoplado
- **Patrón**: Routes → Controllers → Middleware → Database (SQLite)
- **Base de datos**: SQLite con better-sqlite3 (sin ORM, SQL directo)
- **Frontend**: React 19 + JavaScript (sin TypeScript) + Webpack 5
- **Testing**: Sin framework de testing instalado actualmente
...
- Backend: [cambios en controllers, routes, DB]
- Frontend: [cambios en componentes, estado, UI]
- Base de datos: [nuevas tablas, relaciones, índices SQLite]

## Propuesta de Solución
[Diseño técnico siguiendo el patrón Routes → Controllers → Middleware → DB]
...
Siempre proporciona análisis profundos, soluciones bien fundamentadas y documentación clara. No modificar archivos.
