...
function App() {
  ...
  const packs = window.OYS_DATA.packs;

  // Sync tweak darkMode to theme
  useEffect(() => {
    setTheme(tweaks.darkMode ? "dark" : "light");
  }, [tweaks.darkMode]);
  ...
  // Theme toggle from nav also updates tweak
  const handleToggleTheme = () => {
    const newDark = theme !== "dark";
    setTweak("darkMode", newDark);
  };
  ...
  useEffect(() => {
    const anyOpen = openPack || cartOpen || checkoutOpen;
    document.body.style.overflow = anyOpen ? "hidden" : "";
    return () => { document.body.style.overflow = ""; };
  }, [openPack, cartOpen, checkoutOpen]);
  ...
  return (
    <div className="app">
      ...
      {/* Floating cart pill */}
      <button
        className={`cart-fab${cartCount === 0 ? " cart-fab--hidden" : ""}`}
        onClick={() => setCartOpen(true)}
        aria-label={`Ver carrito · ${cartCount} packs · ${formatCLP(cartTotal)}`}
      >
        <span className="cart-fab__icon" aria-hidden="true"><Icon.Cart size={16} /></span>
      ...
      <div className={`toast${toast.visible ? " toast--visible" : ""}`} role="status" aria-live="polite">
        <span className="toast__check"><Icon.Check size={12} /></span>
        {toast.msg}
      </div>
