/* Components.jsx — section components */
...
/* ---------- Nav ---------- */
function Nav({ cartCount, onOpenCart, theme, onToggleTheme, activeSection }) {
  const links = [
  { id: "menu", label: "Menú" },
  ...
  return (
    <nav className="nav glass glass--pill" aria-label="Principal">
      ...
      <div className="nav__links" role="list">
        {links.map((l) =>
        <a key={l.id} href={`#${l.id}`} className={`nav__link${activeSection === l.id ? " nav__link--active" : ""}`}>
            {l.label}
          </a>
        )}
      </div>
  ...
  return (
    <section className="hero" id="top" data-screen-label="01 Hero">
      <div className="hero__media" aria-hidden="true">
        <Placeholder variant="crimson" label="FOTO HERO\nHALLACA EN HOJA DE PLÁTANO\nPRIMER PLANO" />
      </div>
      ...
      <div className="hero__cta-row">
        <button className="btn btn--primary btn--lg" onClick={onShopClick}>
          Ver el menú <Icon.Arrow size={16} />
        </button>
  ...
  return (
    <article
      ...
      <div className="menu-card__media" aria-hidden="true">
        <Placeholder variant={pack.placeholder} label={pack.placeholderText} />
      </div>
      <div className="menu-card__top">
        <span className="menu-card__tag">
          <Icon.Sparkle size={11} /> {pack.tag}
        </span>
        <button
...
function PackModal({ pack, onClose, onAdd }) {
  ...
  React.useEffect(() => {setQty(1);}, [pack?.id]);
  if (!pack) return null;
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal__footer">
          ...
          <a
            ...
            style={{ flex: 1 }}
            href={`https://wa.me/56900000000?text=${encodeURIComponent("Hola Anthony, me interesa el pack +15 hallacas para coordinar.")}`}
            target="_blank"
            rel="noopener"
            onClick={onClose}>
            
              <Icon.Whatsapp size={18} /> Hablar por WhatsApp
            </a>
          }
        </div>
      </div>
              ...
              return (
                <div key={it.id} className="cart-item">
                    <div className="cart-item__thumb">
                      <Placeholder variant={p.placeholder} label="" />
                    </div>
...
/* ---------- Checkout Modal ---------- */
function CheckoutModal({ items, packs, onClose }) {
  ...
  const waUrl = `https://wa.me/56900000000?text=${encodeURIComponent(buildMessage())}`;
        ...
        <div className="modal__body" style={{ paddingTop: 28 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <p style={{ margin: 0, fontSize: 13, color: "var(--fg-muted)" }}>
                Al confirmar, abriremos WhatsApp con este mensaje listo. Anthony te confirmará disponibilidad y cuenta para transferencia.
              </p>
            </div>
          }
        </div>
        ...
        <div className="modal__footer">
          ...
          <>
              ...
              style={{ opacity: canSubmit ? 1 : 0.5 }}>
                ...
                Revisar pedido <Icon.Arrow size={14} />
              </button>
            </> :

          <>
              <button className="btn btn--ghost" onClick={() => setStep(1)}>Atrás</button>
              <a className="btn btn--primary modal__cta" href={waUrl} target="_blank" rel="noopener" onClick={onClose}>
                <Icon.Whatsapp size={16} /> Enviar a WhatsApp
              </a>
            </>
          }
        </div>
        ...
        <div className="tradition__inner">
          <div className="tradition__media">
            <div className="tradition__photo tradition__photo--big">
              <Placeholder variant="default" label="FOTO\nMANOS\nARMANDO\nHALLACA" />
              <span className="tradition__photo-glass">Armado a mano</span>
            </div>
            <div className="tradition__photo">
              <Placeholder variant="coral" label="FOTO\nGUISO" />
            </div>
            <div className="tradition__photo">
              <Placeholder variant="xanthous" label="FOTO\nHOJA DE PLÁTANO" />
            </div>
          </div>
        ...
        <div className="contact__inner">
          <div className="contact-card">
            <div className="contact-card__media" aria-hidden="true">
              <Placeholder variant="default" label="FOTO RETRATO\nANTHONY EN LA COCINA" />
            </div>
            ...
            <div className="contact-card__socials">
              <a className="social-pill" href="#" aria-label="Instagram de Onoto y Sazón">
                <Icon.Instagram size={15} /> Instagram
              </a>
              <a className="social-pill" href="#" aria-label="Facebook de Onoto y Sazón">
                <Icon.Facebook size={15} /> Facebook
              </a>
              <a className="social-pill" href="#" aria-label="X de Onoto y Sazón">
                <Icon.XLogo size={14} /> X
              </a>
            </div>
          </div>
          <div className="contact-info">
            <div className="contact-info__row">
              <span className="contact-info__icon"><Icon.Phone /></span>
              <div>
                <p className="contact-info__label">WhatsApp directo</p>
                <p className="contact-info__value">+56 9 0000 0000</p>
              </div>
            </div>
            <div className="contact-info__row">
              <span className="contact-info__icon"><Icon.MapPin /></span>
              <div>
            ...
            <div className="contact-info__row">
              <span className="contact-info__icon"><Icon.Clock /></span>
              <div>
            ...
            <div style={{ marginTop: 8 }}>
              <a className="btn btn--primary btn--lg" style={{ width: "100%" }} href="https://wa.me/56900000000" target="_blank" rel="noopener">
...
Object.assign(window, {
  formatCLP, Placeholder, Nav, Hero, MenuCard, MenuSection, PackModal, CartModal, CheckoutModal, Tradition, Contact, Footer
});