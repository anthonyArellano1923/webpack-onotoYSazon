...
|---|---|---|
| "Pedidos abiertos · Diciembre 2025" | `src/components/Sections.jsx`, componente `Hero`, dentro de `.hero__eyebrow` | Se actualiza cada temporada. |
...
| Dirección de retiro "Providencia, Santiago" | `src/components/Sections.jsx`, componente `Contact` | |
| Anticipación "48h mínimo · 5 días para packs grandes" | `src/components/Sections.jsx`, componente `Contact` | El "48h" también aparece por separado en la tarjeta de estadísticas del Hero (mismo archivo, componente `Hero`, sección `.hero__stats`) — si cambia la anticipación mínima, revisar los dos lugares. |
